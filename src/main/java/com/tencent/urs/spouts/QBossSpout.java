...
public class QBossSpout extends TdbankSpout {
	...
	public QBossSpout(String config, ImmutableList<Output> outputField) {
	...
	public void processMessage(byte[] message,String categoryId){	
		...
		try {
			...
			}else if(categoryId.equals(Constants.category_adpool) && event_array.length >= 7){
				isEmit = dealPoolMsgByConfig(event_array);
			}
	...
	private boolean dealPoolMsgByConfig(String[] event_array){
		
		String impDate = event_array[0];
		String bid = event_array[1];
		String poolId = event_array[2];
		String poolInfo = event_array[3];
		String recomItems = event_array[4];
		String topItems = event_array[5];
		String ext = event_array[6];
		
		//hash_key,topic,bid,imp_date,cate_id,cate_name,level,father_id
		String[] dealMsg ={poolId,Constants.adpool_stream,bid,impDate,poolId,poolInfo,recomItems,topItems,ext};
